...
import { dictionariesRoutes } from './dictionary/index.ts';
...
import { fileCreateTaskRoute } from './tasks/create.routes.ts';
import { fileExportRoutes } from './tasks/export.ts';
...
export const report6406FileRoutes: FastifyPluginAsync = async (fastify) => {
  await fastify.register(fileCreateTaskRoute, { prefix: '/tasks' });
  await fastify.register(fileExportRoutes, { prefix: '/tasks' });
};
