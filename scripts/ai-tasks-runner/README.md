...
Перед запуском (без `--dry-run`) рабочее дерево должно быть **чистым** — без незакоммиченных изменений. Иначе скрипт завершится с ошибкой (pull и checkout требуют чистого дерева). Сделайте `git stash` или `git add` + `git commit`.
