...
- `GET /api/youtrack/queue` - получить статус очереди операций
- `POST /api/youtrack/queue/process` - обработать все pending операции из очереди вручную

**Примечание:** Очередь операций автоматически обрабатывается при старте приложения, если YouTrack доступен. Если YouTrack недоступен, операции сохраняются в очередь и будут выполнены при следующем запуске.
...
- **Система очереди операций**: при отсутствии доступа к YouTrack операции сохраняются в очередь и выполняются автоматически при следующем запуске приложения
- Очередь операций хранится в файловой системе (`docs/tasks/youtrack-queue/queue.json`) и находится под git-индексацией
