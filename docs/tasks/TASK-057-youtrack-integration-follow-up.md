...
- Реализация `getIssueInfo(youtrackIssueId)` на фронте через новый endpoint (сейчас метод бросает ошибку).
- Предпросмотр задачи YouTrack во вкладке «Связать с существующей»; предпросмотр шаблона во вкладке «Создать задачу в YouTrack»; опциональное переопределение кастомных полей при создании.
