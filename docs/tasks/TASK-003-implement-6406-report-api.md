...
### 1. Структура проекта и именование (28.01.2026)

**Изменение:** Папка `be` переименована в `service2110`

**Обоснование:**
- Более явное и понятное название основного сервиса
- Отделение основного сервиса от вспомогательных инструментов (taskViewer)
- Улучшение читаемости и структуры монорепозитория

**Изменения:**
- Workspace в `package.json`: `be` → `service2110`
- Все скрипты обновлены для работы с `service2110`
- Документация в README обновлена
- Структура папок в документации обновлена

### 2. Упрощение скриптов запуска (28.01.2026)

**Изменение:** Убраны префиксы `be:` из всех скриптов

**До:**
```json
"be:dev": "npm run start:dev -w be",
"be:db:push": "npm run db:push -w be"
```

**После:**
```json
"dev": "npm run start:dev -w service2110",
"db:push": "npm run db:push -w service2110"
```

**Обоснование:**
- Service2110 - основной и единственный backend в проекте
- Более короткие и понятные команды: `npm run dev`, `npm run db:push`
- Task Viewer выделен отдельно как вспомогательный инструмент

### 3. Удаление примеров (Items CRUD) (28.01.2026)

**Изменение:** Полностью удален функционал Items CRUD

**Удалено:**
- `service2110/src/routes/example.ts`
- `service2110/src/schemas/example.schema.ts`
- `service2110/src/services/example.service.ts`
- Таблица `items` из схемы БД
- Тег "Items" из Swagger конфигурации
- Секция "Items (примеры CRUD)" из README

**Обоснование:**
- Оставлен только production-ready код для формы 6406
- Убраны учебные примеры, чтобы не засорять проект
- Чистая кодовая база с фокусом на реальном функционале

### 4. Документация API Conventions (28.01.2026)

**Добавлено:** Создан документ `docs/api-conventions.md`

**Содержание:**
- Правила версионирования API (`/api/v1/*`)
- Структура endpoints для форм отчётности
- Правила именования ресурсов (kebab-case, множественное число)
- HTTP методы и статус коды
- Формат ошибок (RFC 7807)
- Правила пагинации, фильтрации и сортировки
- Примеры правильного и неправильного использования

**Обоснование:**
- Единые стандарты разработки API в проекте
- Документирование архитектурных решений
- Основа для будущих форм отчётности (3462, КРОС и т.д.)

### 5. Seed скрипт для справочников (28.01.2026)

**Добавлено:** Скрипт `service2110/src/db/seed.ts` и команда `npm run db:seed`

**Функционал:**
- Автоматическое заполнение справочников тестовыми данными
- 3 филиала (7701, 7702, 7703)
- 3 источника (SRC001, SRC002, SRC003)
- Поддержка `onConflictDoNothing()` для безопасного повторного запуска

**Обоснование:**
- Упрощение локальной разработки и тестирования
- Быстрое развертывание окружения для новых разработчиков
- Консистентные тестовые данные

### 6. Структура миграций и схем (28.01.2026)

**Реализация:**
- Использование Drizzle ORM для схем
- Префикс `report_6406_` для всех таблиц формы 6406
- Генерация миграций через `drizzle-kit generate`
- Быстрое применение через `drizzle-kit push` (для dev)

**Файлы:**
- `service2110/src/db/schema/report-6406-tasks.schema.ts`
- `service2110/src/db/schema/report-6406-packages.schema.ts`
- `service2110/src/db/schema/report-6406-package-tasks.schema.ts`
- `service2110/src/db/schema/branches.schema.ts`
- `service2110/src/db/schema/sources.schema.ts`
...
### 7. Swagger документация (28.01.2026)

**Реализация:**
- Автоматическая генерация из Zod схем
- Группировка по тегам:
  - `Health` - проверка состояния
  - `Report 6406 - References` - справочники
  - `Report 6406 - Tasks` - задания
  - `Report 6406 - Packages` - пакеты
- Сохранение в `service2110/docs/swagger/swagger.json`
- Доступ через `/docs` (Swagger UI)

**Преимущества:**
- Единый источник истины (Zod схемы)
- Автоматическая синхронизация с кодом
- Интерактивная документация для тестирования

### 8. Организация кода по фичам (28.01.2026)

**Структура для формы 6406:**
```
service2110/src/
...
**Обоснование:**
- Изоляция функционала по формам отчётности
- Легко добавлять новые формы (3462, КРОС)
...
**База данных:**
- 5 таблиц (report_6406_tasks, report_6406_packages, report_6406_package_tasks, branches, sources)
- 8 индексов для оптимизации
- 1 миграция

**API:**
- 20 endpoints (5 справочников + 7 заданий + 8 пакетов)
- 100% покрытие Zod валидацией
