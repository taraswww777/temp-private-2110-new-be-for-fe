...
3. **Новые поля для управления действиями**
   - Добавить `createdBy` (тип: `string`, описание: "ФИО сотрудника, создавшего задание")
...
- [ ] Поле `limit` удалено из request body
- [ ] Добавлено поле `pagination` с типом `$ref: '#/components/schemas/PaginationRequestDto'`
- [ ] Добавлено поле `sorting` с типом `$ref: '#/components/schemas/SortingRequestDto'`
- [ ] Добавлено опциональное поле `filter` (array of `FilterDto`)

### TaskListItemDto создан и применён
- [ ] Создана схема `TaskListItemDto` в `components/schemas`
- [ ] Все существующие поля включены в схему
- [ ] Все ID имеют тип `string` (включая `branchId`)
- [ ] Все даты имеют тип `string` с `format: "date-time"`
- [ ] Все поля имеют описания (`description`)

### Новые поля добавлены
- [ ] Поле `createdBy` (string) добавлено в `TaskListItemDto`
- [ ] Поле `canCancel` (boolean) добавлено в `TaskListItemDto`
- [ ] Поле `canDelete` (boolean) добавлено в `TaskListItemDto`
- [ ] Поле `canStart` (boolean) добавлено в `TaskListItemDto`

### Response обновлён
- [ ] Response 200 использует структуру с `items` и `totalItems`
- [ ] Поле `items` ссылается на массив `TaskListItemDto`
- [ ] Поле `totalItems` имеет тип `integer` с `minimum: 0`

### Валидация
- [ ] OpenAPI спецификация валидна (`swagger-cli validate`)
- [ ] Endpoint корректно отображается в Swagger UI
- [ ] Request и response схемы понятны и читаемы
...
1. Создать новую схему в `components/schemas`:
   ...
   TaskListItemDto:
     ...
     properties:
       ...
       createdBy:
         type: string
         description: "ФИО сотрудника, создавшего задание"
