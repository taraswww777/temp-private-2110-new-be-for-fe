...
**Статус**: ⏳ В работе
**Ветка**: `feature/TASK-071-swagger-local-cache-and-manifest`
...
- **Каталог**: `docs/rawData` (корневой репозиторий)  
  - Принять формат хранения сырых swagger-файлов:
    - `docs/rawData/<snapshotKey>-YYYY-MM-DD.json`  
  ...
  - При успешной загрузке swagger по сети:
    - создавать файл с актуальной датой в `docs/rawData`;
    - сохранять туда полученный JSON с именем `<snapshotKey>-YYYY-MM-DD.json`.
