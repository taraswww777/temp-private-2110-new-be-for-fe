...
## Единообразие URL (форма 6406)

Все эндпоинты API формы 6406 задаются **без конечного слеша** в путях.

- ✅ `/api/v1/report-6406/tasks`, `/api/v1/report-6406/tasks/list`, `/api/v1/report-6406/packages`
- ❌ `/api/v1/report-6406/tasks/`, `/api/v1/report-6406/packages/`

Регистрация роутов, OpenAPI-спецификация и документация используют один и тот же вид путей (без завершающего слеша).
...
### Типизация ответов со списками (OpenAPI)

Ответы с массивами (например, POST /tasks/list, GET /packages) в OpenAPI должны иметь явный тип элементов массива: `items` с `$ref` на DTO элемента (например, `TaskListItemDto`, `PackageDto`), а не обобщённый «array без items».
...
## Поле createdBy (форма 6406)

В ответах API заданий (детали задания, элемент списка) поле `createdBy` **всегда строка, не null**.

- При создании задания пользователем — ФИО сотрудника.
- При автоматическом создании задания (без участия пользователя) — пустая строка `""`.

Контракт: тип `string`, не nullable. См. также [api-6406-requirements-conflicts.md](tasks/api-6406-requirements-conflicts.md).
